...
app.post("/jogadores", async (req, res) => {
  const { nome, vitorias, empate, defesa, gols, infracoes, foto } = req.body

  try {
    const result = await pool.query(
      "INSERT INTO jogadores (nome, vitorias, empate, defesa, gols, infracoes, foto) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
      [nome, vitorias || 0, empate || 0, defesa || 0, gols || 0, infracoes || 0, foto]
    )
...
app.put("/jogadores/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE jogadores SET nome=$1, vitorias=$2, empate=$3, defesa=$4, gols=$5, infracoes=$6, foto=$7 WHERE id=$8 RETURNING *",
      [nome, vitorias, empate, defesa, gols, infracoes, foto, id]
    )
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Jogador não encontrado" })
    }
...
app.post("/goleiros", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO goleiros (nome, vitorias, empate, defesa, gols, infracoes, foto) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
      [nome, vitorias || 0, empate || 0, defesa || 0, gols || 0, infracoes || 0, foto]
    );
...
app.put("/goleiros/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE goleiros SET nome=$1, vitorias=$2, empate=$3, defesa=$4, gols=$5, infracoes=$6, foto=$7 WHERE id=$8 RETURNING *",
      [nome, vitorias, empate, defesa, gols, infracoes, foto, id]
    );
